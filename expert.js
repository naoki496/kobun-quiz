...
(() => {
  ...
  const AUDIO = {
    ...
    tick: new Audio("./assets/sfx/tick.mp3"), // optional; if missing it will be ignored
    timeup: new Audio("./assets/sfx/timeup.mp3"), // optional
  };
  ...
  const state = {
    ...
    timer: {
      t0: 0,
      remainingMs: QUESTION_TIME_SEC * 1000,
      rafId: 0,
      running: false,
      warned: false,
    },
  ...
  boot().catch((err) => {
    console.error(err);
    hardFail("読み込みに失敗しました（CSVまたはファイル配置を確認してください）");
  });

  async function boot() {
    injectMeter();

    // Autostart if ?start=1
    const params = new URLSearchParams(location.search);
    state.modeAutostart = params.get("start") === "1";

    // One-time click to unlock audio
    ...
    if (state.questions.length < TOTAL_QUESTIONS) {
      hardFail(`questions.csv の問題数が不足しています（必要: ${TOTAL_QUESTIONS} / 現在: ${state.questions.length}）`);
      return;
    }
  ...
  function rollCardByRarity(rarity) {
    const pool = rarity === 5 ? state.cards5 : state.cards4;
  ...
  function bumpCombo(reset = false) {
    // Small HUD punch
  ...
  function showOverlay(card, rarity) {
    ...
    if (rarity === 5) {
      el.app.classList.add("fx-correct");
      setTimeout(() => el.app.classList.remove("fx-correct"), 500);
    }
  }
  ...
  function normalizeQuestionRow(row) {
    // Expected headers: id, question, source, choice1..4, answer
  ...
  function normalizeCardRow(row) {
    // Expected headers: id, rarity, name, img, wiki, weight
    const id = clean(row.id);
  ...
  // --- Utils
  function $(sel) {
    const node = document.querySelector(sel.startsWith("#") || sel.startsWith(".") ? sel : `#${sel}`);
    if (!node) throw new Error(`DOM not found: ${sel}`);
    return node;
  }
  ...
  function renderHighlighted(s) {
    // Highlight blocks enclosed by 〖〗 (as your index note says)
    // Escape minimal HTML then wrap highlights.
  ...
  async function unlockAudio() {
    // Attempt to play/pause quickly to unlock on mobile
    try {
      ...
      // Some browsers need a short play
      await AUDIO.bgm.play();
      AUDIO.bgm.pause();
      AUDIO.bgm.currentTime = 0;

      // Start BGM if already in a run
      // (If you prefer silent until first question, comment next line)
      AUDIO.bgm.play().catch(() => {});
    } catch {
      state.audioUnlocked = false;
      el.note.textContent = "音の有効化に失敗（ブラウザ制限の可能性）";
    }
  }
